      ...
      <div className={`flex flex-col rounded-xl relative ${isComplete ? "bg-emerald-100 dark:bg-emerald-500/40 beforeAfterBorder_g" : "bg-yellow-50 dark:bg-yellow-500/40 beforeAfterBorder_y"} select-none`}>
        ...
        <div className={`flex justify-between items-center text-[10px] leading-4 font-medium whitespace-nowrap py-1 px-2 rounded-t-xl border border-b-0 ${isComplete ? "bg-emerald-100 dark:bg-transparent border-emerald-300 text-emerald-500" : "bg-yellow-50 dark:bg-transparent border-yellow-300 text-yellow-500"}`}>
          ...
          <div className='flex items-center gap-2'>
            {/* <span className={`bg-white dark:bg-gray-950 px-2 py-[2px] rounded-[4px] border ${isComplete ? "border-emerald-300 text-gray-300 dark:text-gray-600" : "border-yellow-300 text-gray-600 dark:text-gray-500"}`}>
              Created: {format(new Date(id), 'h:mm a')}
            </span> */}

            <span className={`bg-white dark:bg-gray-950 px-2 py-[2px] rounded-[4px] border ${isComplete ? "border-emerald-300 text-gray-300 dark:text-gray-600" : "border-yellow-300 text-red-500"}`}>
              Expires in: {timeLeft}
            </span>

            <div className={`bg-white dark:bg-gray-950 hover:bg-gray-100 dark:hover:bg-gray-900 px-2 py-[2px] rounded-[4px] border cursor-pointer select-none ${isComplete ? "border-emerald-300 text-gray-300 dark:text-gray-600" : "border-yellow-300 text-gray-600 dark:text-yellow-500"}`}>
        ...
        {/* editBox */}
        {showEditBox && (
          <div className='fixed inset-0 grid place-items-center bg-gray-900 bg-opacity-20 backdrop-blur-sm dark:bg-gray-950 dark:bg-opacity-50 z-[400]'>
            <div className="editBox absolute  min-w-80 max-w-full flex-1 p-2 z-50 rounded-xl border dark:border-gray-900 bg-gray-100 dark:bg-gray-950">
              {/* Editable Text Field */}
              <textarea
                type="text"
                value={editText}
                ...
                className="w-full px-2 py-1 border dark:border-gray-700 outline-none outline-offset-0 flex-1 focus:border-emerald-500 rounded-md text-sm min-h-24 dark:bg-gray-900/50 dark:text-gray-300 dark:focus:border-emerald-500 dark:placeholder:text-gray-400"
              />

              <div className='updateBtns flex gap-2 mt-4 py-1'>
                <div className="p-2.5 bg-gray-950 hover:bg-gray-900 dark:bg-white dark:hover:bg-gray-200 text-xs font-medium text-white rounded-md dark:text-gray-900 whitespace-nowrap">
                  <p onClick={handleSaveClick}>Save Changes</p>
                </div>

                <div className="p-2.5 dark:bg-gray-900 hover:bg-gray-200 bg-white dark:hover:bg-gray-700 text-xs font-medium dark:text-white  text-gray-900 rounded-md whitespace-nowrap">
